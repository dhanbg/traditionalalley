...
export const getEnvironmentDebug = () => {
  const debugInfo = {
    ...
    STRAPI_API_TOKEN: (typeof window === 'undefined' ? (process.env.STRAPI_API_TOKEN || process.env.STRAPI_TOKEN || process.env.NEXT_PUBLIC_STRAPI_API_TOKEN) : process.env.NEXT_PUBLIC_STRAPI_API_TOKEN) ? '[PRESENT]' : '[NOT_CONFIGURED]',
...
export const checkProductionReadiness = () => {
  ...
  if (typeof window === 'undefined' && !(process.env.STRAPI_API_TOKEN || process.env.STRAPI_TOKEN || process.env.NEXT_PUBLIC_STRAPI_API_TOKEN)) {
