...
export async function fetchTopPicksItems() {
  try {
    ...
    const response = await fetchDataFromApi(`/api/top-picks?${TOP_PICKS_POPULATE}`);
    ...
    if (topPicksItem.products && topPicksItem.products.length > 0) {
      for (const product of topPicksItem.products) {
        if (product && product.documentId) {
          const apiEndpoint = `/api/products?filters[documentId][$eq]=${product.documentId}&${PRODUCT_POPULATE}`;
          productFetches.push(
    ...
    if (topPicksItem.product_variants && topPicksItem.product_variants.length > 0) {
      for (const variant of topPicksItem.product_variants) {
        if (variant && variant.documentId) {
          const variantEndpoint = `/api/product-variants?filters[documentId][$eq]=${variant.documentId}&${VARIANT_POPULATE}`;
          variantFetches.push(
              ...
              .then(async variantResponse => {
                if (variantResponse.data && variantResponse.data.length > 0) {
                  ...
                  if (rawVariant.product && rawVariant.product.documentId) {
                    const parentProductEndpoint = `/api/products?filters[documentId][$eq]=${rawVariant.product.documentId}&${PRODUCT_POPULATE}`;
...
export async function fetchSingleProductWithVariants(productId) {
  try {
    ...
    const apiEndpoint = `/api/products?filters[documentId][$eq]=${productId}&${PRODUCT_POPULATE}`;
    return fetchProductsWithVariants(apiEndpoint);
  } catch (error) {
