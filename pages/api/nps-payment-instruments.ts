import type { NextApiRequest, NextApiResponse } from 'next';
import { npsClient } from '../../utils/npsConfig';
import type { 
  GetPaymentInstrumentDetailsRequest, 
  GetPaymentInstrumentDetailsResponse 
} from '../../types/nps';

interface ApiResponse {
  success: boolean;
  message: string;
  data?: GetPaymentInstrumentDetailsResponse;
  error?: string;
}

export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<ApiResponse>
) {
  if (req.method !== 'POST') {
    return res.status(405).json({ 
      success: false, 
      message: 'Method not allowed' 
    });
  }

  try {
    console.log('Getting payment instruments...');

    const requestData: GetPaymentInstrumentDetailsRequest = {
      MerchantId: process.env.NPS_MERCHANT_ID || "7536",
      MerchantName: process.env.NPS_MERCHANT_NAME || "Alley",
      Signature: "placeholder" // Will be calculated based on actual requirements
    };

    console.log('Payment instruments request:', requestData);

    const response = await npsClient.post<GetPaymentInstrumentDetailsResponse>(
      '/GetPaymentInstrumentDetails',
      requestData
    );

    console.log('Payment instruments response:', response.data);

    if (response.data.code === "0") {
      return res.status(200).json({
        success: true,
        message: 'Payment instruments retrieved successfully',
        data: response.data
      });
    } else {
      return res.status(400).json({
        success: false,
        message: response.data.message || 'Failed to get payment instruments',
        error: `Code: ${response.data.code}`
      });
    }

  } catch (error: any) {
    console.error('Error getting payment instruments:', error);

    const errorMessage = error.response?.data?.message || 
                        error.message || 
                        'Internal server error';

    const statusCode = error.response?.status || 500;

    return res.status(statusCode).json({
      success: false,
      message: 'Failed to get payment instruments',
      error: errorMessage
    });
  }
} 