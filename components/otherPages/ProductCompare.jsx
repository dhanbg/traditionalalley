...
import { API_URL, PRODUCT_REVIEWS_API, PRODUCT_POPULATE, COLLECTION_POPULATE } from "@/utils/urls";
        ...
        for (const id of compareItem) {
          try {
            ...
            if (typeof id === 'string' && id.length > 8) {
              ...
              response = await fetchDataFromApi(`/api/products?filters[documentId][$eq]=${id}&${PRODUCT_POPULATE}`);
              ...
              if (response?.data && Array.isArray(response.data) && response.data.length > 0) {
                ...
                response = await fetchDataFromApi(`/api/products/${id}?${PRODUCT_POPULATE}`);
              }
            } else {
            ...
            if (!isDuplicate) {
              ...
              if (attrs.collection && typeof attrs.collection === 'object' && !attrs.collection.category) {
                try {
                  ...
                  const collectionResponse = await fetchDataFromApi(`/api/collections?${COLLECTION_POPULATE}&filters[id][$eq]=${collectionId}`);
