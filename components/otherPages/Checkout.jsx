...
import { fetchDataFromApi, createData, updateData, updateUserBagWithPayment, updateUserBagWithCOD, createOrderRecord, updateProductStock, deleteData } from "@/utils/api";
...
export default function Checkout() {
  ...
  const getUserBagDocumentId = async () => {
    ...
    try {
      ...
      console.log('Creating new user-bag for checkout via proxy:', payload);
      const data = await createData("/api/user-bags", payload);

      if (data?.data?.documentId) {
        return data.data.documentId;
      } else if (data?.data?.id) {
        return data.data.documentId || data.data.id;
      }
